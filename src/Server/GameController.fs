...
let handlers: HttpFunc -> HttpContext -> HttpFuncResult =
    choose [
        ...
        POST >=> route "/" >=>
            fun next ctx ->
                let game = Game.init() |> toPersistedGame <| Guid.NewGuid() |> ctx.GetService<SaveGame>() |> toResponse
                json game next ctx
        GET >=> routef "/%s" (fun id->
            fun next ctx ->
                let game = id |> Guid.Parse |> ctx.GetService<FindGame>() |>toResponse
                json game next ctx
            )
        POST >=> routef "/%s/guess" (fun id ->
            fun next ctx ->
                task {
